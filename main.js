  ...
  try {
    ...
    const matches = await new Promise((resolve) => {
      ...
      if (process.platform === 'darwin') {
        // Spotlight: name is case-insensitive by default
        cmd = 'mdfind';
        args = ['-onlyin', home, '-name', safeName];
      } else if (process.platform === 'linux') {
        // GNU find with depth cap and prune of hidden/junk
        cmd = 'find';
        args = [
          home, '-maxdepth', '6',
          '-type', 'd',
      ...
      const child = execFile(cmd, args, {
        timeout: 4000,
        maxBuffer: 1024 * 1024 // 1MB of output is plenty
      }, (err, stdout) => {
        if (err && err.code !== 'ENOENT') {
          // ENOENT = no matches on some platforms; not actually an error
          // Other errors: log but return what we got
          console.warn('[find-folder-by-name]', err.message);
        }
        const lines = (stdout || '').split('\n').map(s => s.trim()).filter(Boolean);
        for (const line of lines) {
          // mdfind sometimes returns non-folder results that just have
          // matching display names — verify each is actually a directory
          try {
        ...
        // Shortest path first (canonical-location heuristic)
