...
contextBridge.exposeInMainWorld('lunaxDesktop', {
  ...
  // Search for a folder anywhere in the user's home by name (macOS Spotlight
  // / Linux find). Returns { matches: [...paths] }. Empty array if nothing
  // found. Used to auto-resolve folders mentioned in voice commands without
  // forcing the user through the file picker.
  findFolderByName: (name) => ipcRenderer.invoke('find-folder-by-name', name),
